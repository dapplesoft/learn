  ...
  if (!state.course || !state.enrollment) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center bg-white p-4">
        ...
        <Link href="/dashboard" className="text-blue-600 hover:underline flex items-center space-x-2">
  ...
  return (
    <div className={cn(
      ...
      {!isFullscreen && (
        <header className="h-16 border-b border-slate-200 bg-white flex items-center justify-between px-4 shrink-0 shadow-sm">
          <div className="flex items-center space-x-4">
            <Link 
              href="/dashboard" 
              className="flex items-center space-x-2 px-4 py-2 bg-slate-100 hover:bg-slate-200 text-slate-700 rounded-xl transition-all font-bold text-sm"
            >
              <ChevronLeft className="h-4 w-4" />
              <span>Exit Player</span>
            </Link>
            <div className="hidden sm:block">
              <h1 className="font-bold text-sm md:text-base line-clamp-1 text-slate-900">{course.name}</h1>
          ...
          <div className="flex items-center space-x-4">
            <div className="hidden md:flex items-center space-x-2">
              ...
              <span className="text-xs font-bold text-slate-500">{enrollment.progress}%</span>
            </div>
            <button 
      ...
      <div className="flex flex-grow overflow-hidden">
        {/* Main Player Section */}
        <div className="flex-grow flex flex-col bg-slate-50 relative overflow-y-auto scroll-lock">
          <div className={cn(
            ...
            <button 
              ...
              className="absolute bottom-4 right-4 p-2 bg-black/50 hover:bg-black/80 text-white rounded-lg transition-all"
            >
          ...
          <div className="p-6 md:p-10 max-w-4xl mx-auto w-full">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-8">
              <div>
                <h2 className="text-2xl md:text-3xl font-bold mb-2 text-slate-900">{activeVideo.title}</h2>
                <p className="text-slate-500">Duration: {activeVideo.duration}</p>
              </div>
            ...
            <div className="flex justify-between items-center pt-8 border-t border-slate-200">
              <button
                ...
                className="flex items-center space-x-2 text-slate-500 hover:text-blue-600 disabled:opacity-30 disabled:cursor-not-allowed transition-colors"
              >
              ...
              <button
                ...
                className="flex items-center space-x-2 text-blue-600 hover:text-blue-500 disabled:opacity-30 disabled:cursor-not-allowed transition-colors font-bold"
              >
        ...
        <AnimatePresence>
          {sidebarOpen && (
            <motion.aside
              ...
              animate={{ x: 0 }}
              exit={{ x: 300 }}
              className="w-80 border-l border-slate-200 bg-white shrink-0 overflow-y-auto hidden md:block"
            >
                          ...
                          >
                            <div className="shrink-0">
                              {isCompleted ? (
                                ...
                                <Play className={cn("h-5 w-5", isActive ? "text-blue-500" : "text-slate-400")} />
                              )}
                            </div>
